...
module.exports = () => ({
    async createAnonymous(data) {
        const existedData = await strapi.entityService.findMany('plugin::users-permissions.user', {
            filters: { phone: data.phone, role: { name: 'Customer' } },
        });
        if (existedData.length > 0) return { id: existedData[0].id };
    ...
    async createAnonymousWithEmail(data) {
        const existedData = await strapi.entityService.findMany('plugin::users-permissions.user', {
            filters: { email: data.email, role: { name: 'Customer' } },
        });
        if (existedData.length > 0) return { id: existedData[0].id };
